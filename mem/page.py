...
from angr.errors import SimMemoryError, SimSegfaultError
...
class IdaPage(paged_memory.BasePage):
...
class SimIdaMemory(object):
    ...
    def load_objects(self, addr, num_bytes, ret_on_segv=False):
        ...
        for page_addr in self._containing_pages(addr, end):
            ...
            if self.allow_segv and not page.concrete_permissions & IdaPage.PROT_READ:
                #print "... SEGV"
                if ret_on_segv:
                    break
    ...
    def _apply_object_to_page(self, page_base, mo, page=None, overwrite=True):
        ...
        try:
            ...
            if self.allow_segv:
                ...
                raise
        if self.allow_segv and not page.concrete_permissions & IdaPage.PROT_WRITE:
